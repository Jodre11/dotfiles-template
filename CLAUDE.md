...
- `daemon.json` is the only one of the two worth syncing back. If Docker Desktop
  modifies it (e.g. via its settings UI), check the repo copy in
  `docker/.docker/` and remind the user to update it if it has drifted.
- **Never sync `~/.docker/config.json` back into the repo.** Docker Desktop
  writes machine state into it on launch: `auths` accumulates registry
  credentials (for ECR, entries carrying AWS account IDs), and
  `plugins`/`features` regenerate on any machine. There is no tracked
  `config.json` to sync *to* — it is generated by `hydrate.sh` from
  `config.json.tmpl`, which pins `"auths": {}`. Committing the live file would
  push values `.githooks/pre-commit` screens for. A permanent diff is correct.
